...
import { Loader2, BarChart3, ExternalLink, Target, Palette, FileText } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500">输入主题，AI 联网搜集真实数据 → 给出选题方向 + 视觉趋势</p>
        </div>
        ...
        <div className="card-body space-y-3">
          <div className="flex gap-2 flex-wrap items-end">
            ...
            <div>
              ...
              <select className="input" value={platform} onChange={(e) => setPlatform(e.target.value)} disabled={loading}>
        ...
        <>
          {result.queries && result.queries.length > 0 && (
            <div className="text-xs text-slate-500">
              AI 用的搜索词：{result.queries.map((q, i) => <span key={i} className="inline-block bg-slate-100 dark:bg-slate-800 rounded px-2 py-0.5 mr-1">{q}</span>)}
            </div>
          )}

          <div className="card">
            <div className="card-header"><h2 className="font-semibold inline-flex items-center gap-1"><FileText size={16}/> 数据概览</h2></div>
            <div className="card-body whitespace-pre-wrap leading-relaxed text-sm">{a.dataOverview}</div>
          </div>

          {a.directions && a.directions.length > 0 && (
            <div className="card">
              <div className="card-header"><h2 className="font-semibold inline-flex items-center gap-1"><Target size={16}/> 选题方向（{a.directions.length}）</h2></div>
              <div className="card-body space-y-3">
          ...
          <div className="card">
            <div className="card-header"><h2 className="font-semibold inline-flex items-center gap-1"><Palette size={16}/> 视觉风格趋势</h2></div>
            <div className="card-body whitespace-pre-wrap leading-relaxed text-sm">{a.visualTrends}</div>
          </div>

          {a.sources && a.sources.length > 0 && (
            <div className="card">
              <div className="card-header"><h2 className="font-semibold">🔗 参考来源</h2></div>
