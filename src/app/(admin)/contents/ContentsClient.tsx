  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <ContentSignal label="内容总量" value={items.length} note="可复制、审核、发布" />
        <ContentSignal label="草稿" value={draftCount} note="等待打磨" />
        <ContentSignal label="已审核" value={reviewedCount} note="可进入发布" />
        <ContentSignal label="已发布" value={publishedCount} note="运营闭环完成" />
      </section>
      ...
      <ListShell<typeof augmented[number]>
        ...
        viewModes={['card']}
        pageSize={60}
        emptyState="尚无内容。先从任务或平台工作区生成一条内容。"
        ...
        renderCard={(it) => {
          const platLabel = it.platform === 'xiaohongshu' ? '小红书' : '闲鱼';
          const stripColor = it.platform === 'xiaohongshu' ? 'bg-rose-500' : 'bg-amber-500';
          const date = new Date(it.createdAt).toLocaleString('zh-CN');
          return (
            <div className="command-glass detail-lift relative flex h-full flex-col overflow-hidden">
              <div className={`absolute bottom-0 left-0 top-0 w-1 ${stripColor}`} />
              <div className="flex flex-1 flex-col p-4 pl-12">
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span className={it.platform === 'xiaohongshu' ? 'badge-red' : 'badge-yellow'}>{platLabel}</span>
                  <span className="badge-gray">{it.type === 'post' ? '笔记' : '商品'}</span>
                  <span className="badge-blue">{STATUS_LABEL[it.status] ?? it.status}</span>
                  <span className="ml-auto text-slate-400">{date}</span>
                </div>
                <h3 className="mt-3 line-clamp-2 font-bold leading-snug text-slate-900 dark:text-slate-100">{it.title}</h3>
                <p className="mt-2 line-clamp-4 whitespace-pre-wrap text-sm leading-relaxed text-slate-600 dark:text-slate-300">{it.body}</p>
                <div className="mt-auto flex items-center gap-2 pt-4">
                  <button onClick={() => copyItem(it)} className="btn-primary h-8 gap-1.5 px-3 text-xs">
                    <Copy className="h-3.5 w-3.5" aria-hidden />
                    一键复制
                  </button>
                  <select value={it.status} onChange={(e) => setItemStatus(it, e.target.value)} className="input command-input h-8 w-24 py-1 text-xs">
                    <option value="draft">草稿</option>
                    <option value="reviewed">已审核</option>
                    <option value="published">已发布</option>
                  </select>
                  <button onClick={() => delItem(it)} className="ml-auto inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950" aria-label="删除内容">
...
function ContentSignal({ label, value, note }: { label: string; value: number; note: string }) {
  return (
