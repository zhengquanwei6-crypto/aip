// v0.11 B10 · 3 个内置平台说明（写给用户读）
//
// 这些 PlatformInfo 默认用 push.sh 内嵌的 python3 一次性 seed 到 Setting 表（key 前缀 market:platform:），
// 之后从 Setting 表读，允许用户在 /settings 里改（B10 暂未做编辑 UI，留给 v0.10 Chrome 扩展 batch）。
// 读不到时直接 fallback 到本文件的内置版本，保证页面不空。

import type { PlatformInfo } from './types';

export const SEED_PLATFORMS: ReadonlyArray<PlatformInfo> = [
  {
    slug: 'xiaohongshu',
    name: '小红书',
    icon: '📕',
    ...
    dataSource:
      '当前手填（/api/market/trends POST）；未来 v0.10 Chrome 扩展 POST /api/market/trends/sync hook /data-center / 关键词搜索 / 笔记详情接口。',
    recommendedKpis: [
    ...
    recommendedWorkflow:
      '早上 9:30 看一次创作中心 → 抄爆款封面字 → 进 /today 建任务（contentType=案例型 / category=封面图）→ 🎯 全流程发布抽屉 → 三步走 → /contents 复制 + 浏览器手动发。',
  },
  {
    slug: 'xianyu',
    name: '闲鱼',
    ...
    dataSource:
      '当前手填；未来 v0.10 Chrome 扩展 hook 闲鱼卖家中心 m.tb.cn/idle / 2.taobao.com/idle 接口（PC + H5 两套接口都要 hook）。',
    recommendedKpis: [
    ...
    recommendedWorkflow:
      '中午 12:00 看一次卖家中心 → 找浏览高询单低的商品（标题或封面问题）→ 进 /today 建任务（contentType=商品型 / category=商品首图）→ 🎯 全流程发布换图 → 上传回闲鱼。',
  },
  {
    slug: 'qianniu',
    name: '千牛（淘宝/天猫）',
    ...
    dataSource:
      '当前手填；未来 v0.10 Chrome 扩展 hook 千牛工作台 / 生意参谋（sycm.taobao.com）接口。注意：千牛接口反爬比小红书严，建议主用「DOM scrape 兜底」+ hook response 双路径。',
    recommendedKpis: [
    ...
    recommendedWorkflow:
      '每周一 10:00 看生意参谋大盘 → 提炼下周 3 个热词 → 进 /today 给每个客户建一条任务（contentType=案例型 / category=主图）→ 🎯 全流程发布 → 主图候选发给客户选定稿 → 计入 /clients 该客户的月度交付。',
  },
];

const SEED_BY_SLUG = new Map(SEED_PLATFORMS.map((p) => [p.slug, p] as const));

export function getSeedPlatform(slug: string): PlatformInfo | null {
  const v = SEED_BY_SLUG.get(slug as PlatformInfo['slug']);
  return v ?? null;
}
